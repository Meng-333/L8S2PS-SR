...
#                 dir_path = join(file_path, dir)
#                 if os.path.isdir(dir_path):
#                     test_dirs.append(dir_path)  # AID_Test 最后一层类别文件夹
#             elif dir == 'UCM_Test':
#                 dir_path = join(file_path, dir)
#                 if os.path.isdir(dir_path):
#                     test_dirs.append(dir_path)  # UCM_Test 最后一层类别文件夹
#             elif dir == 'NWPU_Test':
#                 dir_path = join(file_path, dir)
#                 class_dirs = sorted(os.listdir(dir_path))
#                 for class_dir in class_dirs:
#                     class_dir_path = join(dir_path, class_dir)
#                     if os.path.isdir(class_dir_path):
#                         test_dirs.append(class_dir_path)  # NWPU_Test 最后一层类别文件夹
#
#     else:
#         test_dirs = datasets
#     input_transform = Compose([  # lr
...
# L8, S2   --- all is 80*80, 240*240 size ---
def get_RGB_trainDataset(data_dir, datasets, crop_size, scale_factor, is_gray=False):
    ...
    for dataset in datasets:
        if dataset == 'train':
            ...
            for dir in dirs:
                dir_path = join(file_path, dir)
                if dir == 'train_HR':
                    HR_train_dirs.append(dir_path)  # HR 最后一层类别文件夹
                if dir == 'train_LR':
                    LR_train_dirs.append(dir_path)  # LR 最后一层类别文件夹


    return RGB_TrainDatasetFromFolder(HR_train_dirs,
                                  LR_train_dirs,
                                  is_gray=is_gray,
                                  random_scale=True,    # random scaling
                                  crop_size=crop_size,  # random crop
...
def get_RGB_testDataset(data_dir, datasets, crop_size, scale_factor, is_gray=False):
    ...
    if dataset == 'test':
        ...
        for dir in dirs:
            dir_path = join(file_path, dir)
            if dir == 'test_HR':
                HR_test_dirs.append(dir_path)  # HR 最后一层类别文件夹
            if dir == 'test_LR':
                LR_test_dirs.append(dir_path)  # LR 最后一层类别文件夹
    else:
        test_dirs = datasets
    input_transform = Compose([  # lr
        # CenterCrop(crop_size),
    ...
    return RGB_DatasetFromFolder2(HR_test_dirs, LR_test_dirs,
                                  input_transform=input_transform, input_resize_transform=srcnn_input_transform,
                                   target_transform=target_transform, is_gray=is_gray, scale_factor=scale_factor)