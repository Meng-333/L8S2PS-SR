...
class RGB_TrainDatasetFromFolder(data.Dataset):
    def __init__(self, image_dirs_HR, image_dirs_LR, is_gray=False, random_scale=True, crop_size=216, rotate=True, fliplr=True,
                 fliptb=True, scale_factor=3):
        super(RGB_TrainDatasetFromFolder, self).__init__()

        self.HR_image_filenames = []
        self.LR_image_filenames = []
        for image_dir in image_dirs_HR:
            self.HR_image_filenames.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if is_image_file(x))
        for image_dir in image_dirs_LR:
            self.LR_image_filenames.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if is_image_file(x))
        #print('train_dataset---------self.image_filenames:', self.image_filenames, len(self.image_filenames))
        self.is_gray = is_gray
        self.random_scale = random_scale
        self.crop_size = crop_size
    ...
    def __getitem__(self, index):
        # load image
        img_HR = load_img(self.HR_image_filenames[index])
        img_LR = load_img(self.LR_image_filenames[index])
        ...
        hr_img = functional.to_tensor(img_HR)
        lr_img = functional.to_tensor(img_LR)
        ...
        # bc_transform = Compose([ToPILImage(), Resize((hr_img_w, hr_img_h), interpolation=Image.BICUBIC), ToTensor()])
        # bc_img = bc_transform(lr_img)
        bc_img = functional.resize(img_LR, size=[hr_img_w, hr_img_h], interpolation=Image.BICUBIC)
        bc_img = functional.to_tensor(bc_img)

        return lr_img, hr_img, bc_img, self.HR_image_filenames[index]
...
class RGB_DatasetFromFolder2(data.Dataset):
    def __init__(self, HR_image_dirs, LR_image_dirs, input_transform=None, input_resize_transform=None, target_transform=None, is_gray=False, scale_factor=3, max_sample_num=100000):
        super(RGB_DatasetFromFolder2, self).__init__()
        #self.image_filenames = [join(image_dir, x) for x in listdir(image_dir) if is_image_file(x)]
        HR_image_files = []
        LR_image_files = []
        for image_dir in HR_image_dirs:
            HR_image_files.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if is_image_file(x))
        for image_dir in LR_image_dirs:
            LR_image_files.extend(join(image_dir, x) for x in sorted(listdir(image_dir)) if is_image_file(x))

        self.HR_image_filenames = HR_image_files
        self.LR_image_filenames = LR_image_files
        #print('test_dataset--------------self.image_filenames:', self.image_filenames, len(self.image_filenames))
        self.input_transform = input_transform  # lr
        self.input_resize_transform = input_resize_transform   # bicubic
        self.target_transform = target_transform   # hr
    ...
    def __getitem__(self, index):
        input_HR = load_img(self.HR_image_filenames[index])
        input_LR = load_img(self.LR_image_filenames[index])
        # only Y-channel is super-resolved
        # if self.is_gray:
        #     input = input.convert('YCbCr')
        target = input_HR.copy()
        input_resize = input_LR.copy()
        input = input_LR.copy()
        if self.input_transform:                  # lr
            input = self.input_transform(input_LR)
        if self.input_resize_transform:           # bic
            input_resize = self.input_resize_transform(input_resize)
        if self.target_transform:                 # hr
            target = self.target_transform(target)

        return input, target, input_resize, self.HR_image_filenames[index]
